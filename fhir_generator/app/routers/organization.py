...
from app.modules import OrganizationResourceLtc
...
async def generate_ltc_tw_organization(item: OrganizationLTC):
    status_code = 200
    resource_name = 'Organization'
    item_dict = item.model_dump()
    organization_resource = {}
    ...
    try:
        organization = OrganizationResourceLtc.OrganizationResourceLtc(resource_name, item_dict)
        organization_resource = organization.generate_organization_resource()
    except Exception as e:
        status_code = 500
    ...
    return JSONResponse(
        {
            'status': status_code,
            'message': 'Creating Organization resource for LTC is successful.',
            'data': [organization_resource],
        },
        status_code=status_code
    )
