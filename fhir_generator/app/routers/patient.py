...
from app.modules import PatientResourceLtc
...
async def generate_ltc_tw_patient(request: Request, item: PatientLTC):
    status_code = 200
    resource_name = 'Patient'
    item_dict = item.model_dump()
    patient_resource = {}
    ...
    return JSONResponse(
        {
            'status': status_code,
            'message': 'Creating Patient resource for LTC is successful.',
            'data': [patient_resource],
        },
        status_code=status_code
    )
