...
while True:
    ...
    if error_counter > 5:
        json_dict['resource_name'] = 'InvalidState'

    error_counter += 1
    response = httpx.post(
        'http://fhir_generator:8000/circuit/api/v1/ltc_tw_2025_location_broken',
        headers=headers,
        json=json_dict
    )
    ...
    if response.status_code == 200:
        ...
        records = [
            [
                ...
                str(response.status_code), 'fhir_generator', 'ltc_tw_2025_location_broken',
            ],
        ]
